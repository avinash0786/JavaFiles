...
public class sortingClass {
    ...
    public static void mergeSort(int[] arr, int l, int r) {
        ...
        System.out.println("Cancelled call low: " + l + " high: " + r);
    }
...
//            System.out.println("Low: "+low+" Mid: "+mid+" high: "+high);
    ...
    public static ArrayList<Integer> threeWayPartitionGFG(ArrayList<Integer> A, int a, int b)
    {
        int low=0;
        int mid=0;
        int high=A.size()-1;
        while (mid<=high){
            ...
            else if(A.get(mid)>=a && A.get(mid)<=b){
...
//        System.out.println("low: "+(low*(low+1)/2));
    ...
    static void sortABS(int arr[], int n,int k)
    {
        test[] bb=new test[n];
        for (int i = 0; i < n; i++) {
            bb[i]=new test(arr[i],k);
        }
        Arrays.sort(bb,test::compareTo);
        int i=0;
        for (test test : bb) {
            arr[i++]=test.var;
        }
    ...
    static long countPairs(int x[], int y[], int n, int m)
    {
        ...
        for (int i = 0; i < n; i++) {
            if(x[i]==0)
                ...
                int indx=0;
                while (y[indx]<x[i]){
                    if(y[indx]==3 || y[indx]==4){
                        res--;
                    }
                    indx++;
                }
                System.out.println("X: "+x[i]+"  y: "+y[indx]+" indx: "+indx);
                res+=(m-indx);
            }
            else {
                int indx=0;
                ...
                res+=(m-indx);
                System.out.println("Else-X: "+x[i]+"  y: "+y[indx]+" indx: "+indx);
            }
        ...
        for (int i = 0; i < n - 2; ++i) {
            ...
            for (int j = i + 1; j < n; ++j) {
                ...
                while (k < n && arr[i] + arr[j] > arr[k]) {
                    System.out.println("a: "+arr[i]+" b: "+arr[j]+" > c: "+arr[k]+" (i: "+i+" ,j: "+"j: "+j+" , k: "+k+")");
                    ++k;
                }
                if (k > j)
                    count += k - j - 1;
...
//        System.out.println(maxGuest(new int[]{800,850,1000,1200},new int[]{900,950,1123,1340}));
//        System.out.println(kSmallest(new int[]{5,6,2,8,4,9},2));
...
//        rangePartition(new int[]{1,2,3,3,4},1,2);
//        ArrayList<Integer> op=new ArrayList<Integer>(Arrays.asList(1,2,3,3,4));
//        System.out.println();
//        System.out.println(threeWayPartitionGFG(op,1,2));
//        System.out.println();
...
class test implements Comparable<test>
{
    int var;
    static int k;
    public test(int var,int k){
        this.var=var;
        test.k =k;
    }

    public int compareTo( test o) {
        int b=Math.abs(test.k-o.var);
        int a=Math.abs(test.k-this.var);
        if(a>b)return 1;
